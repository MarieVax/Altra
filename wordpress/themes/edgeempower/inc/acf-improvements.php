...
function image_from_id($id, $options = array() ) {
  ...
  if( $id ) {
    if($config['preview']) {
      ...
      // Load standard image with native lazy load
      if($config['lazyload'] == 'native') {
        $classes = $config['class'] . ' skip-lazy';
        echo wp_get_attachment_image( $id, $config['size'], false, array( 'class' => $classes, 'loading' => 'lazy' ));
      }
      ...
      if($config['lazyload'] == 'swiper') {
        $classes_swiper = 'skip-lazy ' . $config['class'];
        $img_markup = wp_get_attachment_image( $id, $config['size'], false, array( 'class' => $classes_swiper ));
        $img_markup = str_replace('src=', 'loading="lazy" src=', $img_markup);
        echo $img_markup;
      ...
      if($config['lazyload'] == 'swiper-simple') {
        ...
        $img_src = $img_data[0];
        $img_alt = get_post_meta($id, '_wp_attachment_image_alt', true);
        $classes = 'skip-lazy '; // skip-lazy to disable lazysizes plugin
        if(!empty($config['class'])) {
          $classes .= ' ' . $config['class'];
        }
